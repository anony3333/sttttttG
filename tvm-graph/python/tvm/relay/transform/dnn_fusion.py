...
from .utils import ReplaceParams, is_scalar, EmitTuple
...
class GroupOp:
    ...
    def init_args(self, group: List[relay.Call]) -> List[ExprWithOp]:
        ...
        for node in group:
            for arg in node.args:
                if isinstance(arg, relay.TupleGetItem) and (arg.tuple_value in group):
                    continue
    ...
    def emit_tuple(self, expr: relay.Expr) -> relay.Expr:
        return EmitTuple().visit(expr)

    def flatten_expr(self, expr: relay.Expr, params: List[relay.Var]) -> relay.Expr:
        if isinstance(expr, relay.Tuple):
            func1_args = []
            for arg in expr.fields[1].args:
                if isinstance(arg, relay.TupleGetItem):
                    func1_args.append(arg.tuple_value)
                else:
                    func1_args.append(arg)
            if expr.fields[0] in func1_args:
                expr = self.special_serial_flatten(expr, params)
            else:
                expr = self.parallel_flatten(expr, params)
        else:
            expr = self.serial_flatten(expr, params)

        return self.emit_tuple(expr)

    def flatten_func(self, expr: relay.Call) -> relay.Expr:
        body = expr.op.body
        replace_map = {}
        for used_arg, func_param in zip(expr.args, expr.op.params):
            replace_map[func_param] = used_arg
        return ReplaceParams(replace_map).visit(body)

    def parallel_flatten(self, expr: relay.Expr, params: List[relay.Var]) -> relay.Expr:
        assert isinstance(expr, relay.Tuple)
        tuple_expr: relay.Tuple = expr
        assert len(tuple_expr) == 2

        first_func, second_func = tuple_expr[0], tuple_expr[1]
        if isinstance(first_func.op, relay.Function):
            first_func = self.flatten_func(first_func)
        if isinstance(second_func.op, relay.Function):
            second_func = self.flatten_func(second_func)

        ret_tensor = []

        first_type = relay.transform.InferTypeLocal(first_func)
        if isinstance(first_type, tvm.ir.type.TupleType):
            for i in range(len(first_type.fields)):
                ret_tensor.append(relay.TupleGetItem(first_func, i))
        else:
            ret_tensor.append(first_func)

        second_type = relay.transform.InferTypeLocal(second_func)
        if isinstance(second_type, tvm.ir.type.TupleType):
            for i in range(len(second_type.fields)):
                ret_tensor.append(relay.TupleGetItem(second_func, i))
        else:
            ret_tensor.append(second_func)
        return relay.Tuple(ret_tensor)

    def special_serial_flatten(self, expr: relay.Expr, params: List[relay.Var]) -> relay.Expr:
        # flatten func1
        func1 = expr.fields[0]
        if isinstance(func1.op, relay.Function):
            new_func1 = self.flatten_func(func1)
            replace = ReplaceParams({})
            replace.memo_map[func1] = new_func1
            new_func2 = replace.visit(expr.fields[1])
        else:
            new_func1 = func1
            new_func2 = expr.fields[1]
        
        if isinstance(new_func2.op, relay.Function):
            new_func2 = self.flatten_func(new_func2)

        ret_tensor = []
        first_type = relay.transform.InferTypeLocal(new_func1)
        if isinstance(first_type, tvm.ir.type.TupleType):
            for i in range(len(first_type.fields)):
                ret_tensor.append(relay.TupleGetItem(new_func1, i))
        else:
            ret_tensor.append(new_func1)

        second_type = relay.transform.InferTypeLocal(new_func2)
        if isinstance(second_type, tvm.ir.type.TupleType):
            for i in range(len(second_type.fields)):
                ret_tensor.append(relay.TupleGetItem(new_func2, i))
        else:
            ret_tensor.append(new_func2)
        return relay.Tuple(ret_tensor)

    def serial_flatten(self, expr: relay.Expr, params: List[relay.Var]) -> relay.Expr:
        # flatten first fuse op
        global_replace = {}
        for arg in expr.args:
            if arg not in params and isinstance(arg, relay.TupleGetItem):
                tuple_value = arg.tuple_value
                if not isinstance(tuple_value.op, relay.Function):
                    continue
                elif tuple_value in global_replace:
                    continue
                new_tuple = self.flatten_func(tuple_value)
                global_replace[tuple_value] = new_tuple
            elif arg not in params and isinstance(arg.op, relay.Function):
                replace_map = {}
                body = arg.op.body
                for used_arg, func_param in zip(arg.args, arg.op.params):
                    replace_map[func_param] = used_arg
                body = ReplaceParams(replace_map).visit(body)
                global_replace[arg] = body
        if global_replace:
            expr = ReplaceParams(global_replace).visit(expr)

        # flatten last fuse op
        if isinstance(expr.op, relay.Function):
            body = expr.op.body
            replace_map = {}
            for used_arg, func_param in zip(expr.args, expr.op.params):
                replace_map[func_param] = used_arg
            expr = ReplaceParams(replace_map).visit(body)
        return expr
    ...
    def fuse(self) -> None:
        ...
        ret_expr = self.flatten_expr(ret_expr, params)
        ret_type = relay.transform.InferTypeLocal(ret_expr)
        func = Function(params, ret_expr, ret_type, [])
        fuse_tag = []
        for call in self.group:
            if hasattr(call.attrs, "tag"):
                fuse_tag.append(call.attrs.tag.replace("stencil_", ""))
            elif hasattr(call.op.attrs, "tag"):
                fuse_tag += call.op.attrs.tag.replace("fuse_", "").split("_")
            else:
                raise ValueError("can't get tag", call)
        func = func.with_attr({"tag": "fuse_" + "_".join(sorted(set(fuse_tag)))})
