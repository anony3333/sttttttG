...
class Call(ExprWithOp):
    ...
    def __str__(self):
        if hasattr(self.attrs, "tag"):
            return f"Stencil({self.attrs.tag})"
        elif hasattr(self.op.attrs, "tag"):
            return f"FuseStencil({self.op.attrs.tag})"
        return super().__str__()

    def __repr__(self):
        if hasattr(self.attrs, "tag") or hasattr(self.op.attrs, "tag"):
            return self.__str__()
