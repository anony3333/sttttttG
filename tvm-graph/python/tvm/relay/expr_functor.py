...
class MixedModeMutator(ExprMutator):
    """Non-recursive DFS Graph Traversal for Custom Rewriting Passes"""

    def __init__(self, pre: bool = False):
        super(MixedModeMutator, self).__init__()
        self.pre_ = pre
    
    
    """ Rewrite Related """
    def rewrite_default(self, op, post):
        return post

    def rewrite_tuple(self, op, post):
        raise NotImplementedError()

    def rewrite_call(self, op, post):
        raise NotImplementedError()
    
    def rewrite_tuple_getitem(self, op, post):
        raise NotImplementedError()

    def rewrite(self, op):
        post = ExprMutator().visit(op)
        if isinstance(op, Tuple):
            return self.rewrite_tuple(op, post)
        elif isinstance(op, Call):
            return self.rewrite_call(op, post)
        elif isinstance(op, TupleGetItem):
            return self.rewrite_tuple_getitem(op, post)
        elif hasattr(self, "rewrite_" + op.__class__.__name__.lower()):
            return getattr(self, "rewrite_" + op.__class__.__name__.lower())(op, post)
        else:
            return self.rewrite_default(op, post)
    
    def visit_tuple(self, op):
        return self.rewrite(op)
    
    def visit_call(self, call_node):
        return self.rewrite(call_node)
    
    def visit_tuple_getitem(self, op):
        return self.rewrite(op)


def ExpandANormalForm(let_node, pre_visit, post_visit):
    """ copy from src/relay/ir/expr_functor.cc: ExpandANormalForm()"""
    stack = []
    stack.append(let_node)
    is_anormal = True
    while is_anormal:
        current_op = stack[-1]
        pre_visit(current_op)
        if isinstance(current_op.body):
            stack.push(current_op.body)
        else:
            is_anormal = False
    while stack:
