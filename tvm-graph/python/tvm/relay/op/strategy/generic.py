...
from .stencil_dispatch import stencil_register
...
def wrap_compute_stencil_test(topi_compute):
    """ Wrap stencil topi compute """
    
    def _compute_stencil(attrs, inputs, out_type):
        return [topi_compute(inputs[0], inputs[1], False)]
    return _compute_stencil


@override_native_generic_func("stencil_2d2r_strategy")
def stencil_2d2r_strategy(attrs, inputs, out_type, target):
    """ stencil star2d2r strategy """
    strategy = _op.OpStrategy()
    strategy.add_implementation(
        wrap_compute_stencil_test(topi.stencil.star2d2r),
        wrap_topi_schedule(topi.generic.schedule_stencil_star2d2r),
        name="stencil.star2d2r",
    )
    return strategy

def wrap_compute_stencil(topi_compute):
    """ Wrap stencil topi compute """
...
@override_native_generic_func("stencil_1d4r_strategy")
def stencil_1d4r_strategy(attrs, inputs, out_type, target):
    """ stencil star1d4r strategy """
    strategy = _op.OpStrategy()
    regular = attrs.regular
    if regular:
        strategy.add_implementation(
            wrap_compute_stencil(topi.stencil.star1d4r_regular),
            wrap_topi_schedule(topi.generic.schedule_stencil_star1d4r),
            name="stencil.star1d4r",
        )
    else:
        tag = attrs.tag
...
@override_native_generic_func("stencil_1grid_strategy")
def stencil_1grid_strategy(attrs, inputs, out_type, target):
    """ stencil 1 grid stencil strategy """
    strategy = _op.OpStrategy()
    tag = attrs.tag

    compute = stencil_register.get_compute(tag)
    schedule = stencil_register.get_schedule(tag)

    strategy.add_implementation(
        wrap_compute_stencil(compute),
        wrap_topi_schedule(schedule),
        name=tag,
    )
...
@override_native_generic_func("stencil_2grid_strategy")
def stencil_2grid_strategy(attrs, inputs, out_type, target):
    """ stencil 2 grid stencil strategy """
    strategy = _op.OpStrategy()
    tag = attrs.tag
...
@override_native_generic_func("stencil_3grid_strategy")
def stencil_3grid_strategy(attrs, inputs, out_type, target):
    """ stencil 3 grid stencil strategy """
    strategy = _op.OpStrategy()
    tag = attrs.tag
...
@override_native_generic_func("stencil_4grid_strategy")
def stencil_4grid_strategy(attrs, inputs, out_type, target):
    """ stencil 4 grid stencil strategy """
    strategy = _op.OpStrategy()
    tag = attrs.tag
...
@override_native_generic_func("stencil_5grid_strategy")
def stencil_5grid_strategy(attrs, inputs, out_type, target):
    """ stencil 5 grid stencil strategy """
    strategy = _op.OpStrategy()
    tag = attrs.tag
...
@override_native_generic_func("stencil_6grid_strategy")
def stencil_6grid_strategy(attrs, inputs, out_type, target):
    """ stencil 6 grid stencil strategy """
    strategy = _op.OpStrategy()
    tag = attrs.tag
...
@override_native_generic_func("stencil_2mogrid_strategy")
def stencil_2mogrid_strategy(attrs, inputs, out_type, target):
    """ stencil 2 grid multi-output stencil strategy """
    strategy = _op.OpStrategy()
    tag = attrs.tag
