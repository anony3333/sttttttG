...
class MLIRCodeGen:
    ...
    def repeat_access(self, body_list):
        ...
        def search_related_line(root_var):
            ...
            while var_list:
                ...
                var_list += deque(var_compute.get(var, []))
        ...
        # add offset
        def add_offset(related_line, offset):
            offseted_line = []
        ...
        for line in wait_repeat_access:
            ...
            offset_code[line] = add_offset(access_var[var], offset)

        # get ret var
        ret_line = fuse_body[-1]
        ret_vars = [token for token in ret_line.split() if token.startswith("%")]
        # replace code
        line_idx_map = {}
        replace_var = {}
    ...
    def print_output(self, func, f=0):
        if len(self.ret_data) == 1:
            print(self.ret_data[0]) if f==0 else f.write(self.ret_data[0]+'\n')
            return
        ...
        for i, line in enumerate(self.ret_data):
            ...
            if isinstance(var, relay.TupleGetItem):
                tag_map[tag] = self.variable2name[var.tuple_value] + "#" + str(var.index)
                if f==0:
                    print(line.replace(tag, tag_map[tag]))
                else:
                    f.write(line.replace(tag, tag_map[tag])+'\n')
            else:
                tag_map[tag] = tag
                if f==0:
                    print(line)
    ...
    def PRINT_CALL_MLIR(self, call, MESHSIZE, HALO, MLIR_PATH, call_shape_infer):
        ...
        print('func @kernel_'+GET_CALL_TAG(call)+'('+program_data.split('(')[-1])
        ...
        mycast_list = []
        for name in call_input_list+call_output_list:
            t = '('+PRINT_BRA(-HALO,-HALO,-HALO)+' : ' + PRINT_BRA(MESHSIZE-HALO,MESHSIZE-HALO,MESHSIZE-HALO)+ ')' + ' : (!stencil.field<?x?x?xf64>) -> !stencil.field'+ PRINT_ANG(str(MESHSIZE),str(MESHSIZE),str(MESHSIZE),'f64')
        ...
        for name in call_input_list:
            t = ':'+'(!stencil.field'+ PRINT_ANG(str(MESHSIZE),str(MESHSIZE),str(MESHSIZE),'f64') +') -> !stencil.temp<?x?x?xf64>' #TODO:shape未对应，先留着
            myload_list.append('%'+name+' = stencil.load '+ '%cast_'+name + t)
        print("\n".join(myload_list))

        print("\n")
        print(self.print_code(call,1))
        ...
        for name in call_output_list:
            ...
            t = call_shape_infer[call]+ ': !stencil.temp<?x?x?xf64> to !stencil.field' + PRINT_ANG(str(MESHSIZE),str(MESHSIZE),str(MESHSIZE),'f64')
            if isinstance(call.checked_type, relay.TupleType):
    ...
    def shape_infer(self,func,MLIR_PATH):
        ...
        #得到原始stencil的所有shape
        MLIRNAME = "codegen_"+self.mlir_name+"_1func.mlir"
        ...
        for l in streamlist:
            for ele in l: 
                if isinstance(ele, tvm.relay.expr.Call):
                    self.PRINT_CALL_MLIR(ele, MESHSIZE, HALO, MLIR_PATH, call_shape_infer)
                else:
                    print('***************'+ele+'***************')
