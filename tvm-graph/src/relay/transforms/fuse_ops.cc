...
Expr CustomFuseOps(const Expr& expr, int fuse_opt_level, size_t max_fuse_depth, bool link_params,
             const IRModule& module) {
  LOG(INFO) << "in custom fuse ops";
  return FuseMutator(fuse_opt_level, max_fuse_depth, link_params).Transform(expr);
}
...
Pass CustomFuseOps(int fuse_opt_level) {
  runtime::TypedPackedFunc<Function(Function, IRModule, PassContext)> pass_func =
      [=](Function f, IRModule m, PassContext pc) {
        bool link_params = false;
        Executor executor =
        ...
        int opt_level = fuse_opt_level == -1 ? pc->opt_level : fuse_opt_level;
        auto max_fuse_depth = pc->GetConfig("relay.FuseOps.max_depth", Integer(kMaxFusedOps));
        return Downcast<Function>(CustomFuseOps(f, opt_level, max_fuse_depth.value(), link_params, m));
      };
  return CreateFunctionPass(pass_func, 0, "FuseOps", {"InferType"});
}

TVM_REGISTER_GLOBAL("relay._transform.FuseOps").set_body_typed(FuseOps);
TVM_REGISTER_GLOBAL("relay._transform.CustomFuseOps").set_body_typed(CustomFuseOps);
